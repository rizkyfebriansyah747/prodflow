...
// Setiap konten punya assignees array. Tiap assignee punya status sendiri.
// Status siklus: aktif → review (setelah submit) → selesai/revisi (setelah PIC review)
// Konten berstatus 'review' jika SEMUA assignee sudah submit (semua review/selesai/revisi)
// Konten berstatus 'selesai' jika semua assignee selesai
const SEED_EVENTS = [
  { id:1, title:'Reels IG — Kenapa Paspor Penting?', cat:'Instagram', loadType:'utama', start:'2025-04-19', end:'2025-04-21', pic:'u2',
    brief:'Buat reels menarik tentang pentingnya paspor bagi traveler pemula.',
    assignees:[
      { userId:'u4', role:'Cameraman',   status:'review',  submitLink:'https://drive.google.com/sample-footage', submitNote:'Raw footage sudah diupload, 12 menit total.' },
      { userId:'u5', role:'Desain Cover', status:'aktif',   submitLink:null, submitNote:null },
    ]},
  { id:2, title:'TikTok — Tips Hemat Belanja Bulanan', cat:'TikTok', loadType:'utama', start:'2025-04-21', end:'2025-04-23', pic:'u3',
    brief:'Konten tips hemat yang relatable untuk anak muda.',
    assignees:[
      { userId:'u4', role:'Talent',   status:'aktif', submitLink:null, submitNote:null },
      { userId:'u6', role:'Scripting',status:'aktif', submitLink:null, submitNote:null },
    ]},
  { id:3, title:'Artikel: Cara Urus Visa Schengen', cat:'Artikel', loadType:'utama', start:'2025-04-22', end:'2025-04-25', pic:'u2',
    brief:'Panduan lengkap pengurusan visa Schengen step by step.',
    assignees:[
      { userId:'u5', role:'Penulis', status:'aktif', submitLink:null, submitNote:null },
    ]},
  { id:4, title:'YouTube — Travel Vlog Jepang', cat:'YouTube', loadType:'event', start:'2025-04-23', end:'2025-04-30', pic:'u3',
    brief:'Vlog perjalanan Jepang 7 hari, fokus kuliner dan budaya.',
    assignees:[
      { userId:'u4', role:'Editor Video', status:'aktif', submitLink:null, submitNote:null },
      { userId:'u5', role:'Desain Cover', status:'aktif', submitLink:null, submitNote:null },
    ]},
  { id:5, title:'Podcast — Eps. 12 Tips Traveling', cat:'Podcast', loadType:'utama', start:'2025-04-17', end:'2025-04-20', pic:'u3',
    brief:'Episode podcast dengan narasumber travel blogger.',
    assignees:[
      { userId:'u5', role:'Editor Video', status:'revisi', submitLink:'https://drive.google.com/podcast-eps12',
        submitNote:'Sudah diedit, mohon dicek bagian intro.', revisionNote:'Intro terlalu panjang, potong di menit ke-2. Audio bagian tengah kurang jelas.' },
    ]},
  { id:6, title:'IG Story — Promo Event Mei', cat:'Instagram', loadType:'utama', start:'2025-04-24', end:'2025-04-26', pic:'u2',
    brief:'Story promosi untuk event offline bulan Mei.',
    assignees:[
      { userId:'u5', role:'Desainer', status:'aktif', submitLink:null, submitNote:null },
    ]},
  { id:7, title:'Festival Kuliner Jakarta 2025', cat:'Event', loadType:'event', start:'2025-04-27', end:'2025-05-03', pic:'u2',
    brief:'Liputan event festival kuliner tahunan di Jakarta.',
    assignees:[
      { userId:'u4', role:'Cameraman',  status:'aktif', submitLink:null, submitNote:null },
      { userId:'u6', role:'Fotografer', status:'aktif', submitLink:null, submitNote:null },
    ]},
  { id:8, title:'TikTok — OOTD Ramadan', cat:'TikTok', loadType:'utama', start:'2025-04-14', end:'2025-04-16', pic:'u3',
    brief:'Konten fashion Ramadan yang trending.',
    assignees:[
      { userId:'u4', role:'Talent',   status:'selesai', submitLink:'https://drive.google.com/ootd', submitNote:'Sudah selesai.' },
      { userId:'u5', role:'Desainer', status:'selesai', submitLink:'https://drive.google.com/cover', submitNote:'Cover sudah dibuat.' },
    ]},
]
...
function LoginPage({ users, onLogin }) {
  ...
  const demos=[['manager@tikkim.id','Manager','🏛'],['rina@tikkim.id','PIC Instagram & Artikel','📸'],['budi@tikkim.id','PIC TikTok & Podcast','🎵'],['inul@tikkim.id','Tim Produksi (Intern)','🎬'],['redul@tikkim.id','Tim Produksi (Intern)','✏️']]
  return (
    <div style={{display:'flex',minHeight:'100vh',background:C.bg,fontFamily:'Poppins,sans-serif'}}>
      <div style={{width:440,background:'#003580',display:'flex',flexDirection:'column',justifyContent:'center',padding:'52px 48px',position:'relative',overflow:'hidden'}}>
        ...
        <div style={{position:'absolute',bottom:-40,left:-60,width:180,height:180,borderRadius:'50%',background:'rgba(255,215,0,.04)'}}/>
        <div style={{position:'relative'}}>
          ...
          <div style={{fontSize:21,fontWeight:700,color:'#fff',marginBottom:6,lineHeight:1.3}}>Masuk ke akun kamu</div>
          <div style={{fontSize:13,color:'rgba(255,255,255,.4)',marginBottom:30}}>Platform manajemen konten tim produksi</div>
          <div style={{marginBottom:13}}>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:8,maxWidth:380}}>
          {demos.map(([e,label,icon])=>(
            <button key={e} onClick={()=>{setEmail(e);setPass('1234');setTimeout(()=>{const u=users.find(x=>x.email===e);if(u)onLogin(u)},80)}}
              style={{textAlign:'left',padding:'11px 15px',border:`1px solid ${C.border}`,borderRadius:11,background:C.card,cursor:'pointer',fontFamily:'Poppins,sans-serif',fontSize:13,color:C.text,boxShadow:C.shadow,display:'flex',alignItems:'center',gap:10}}>
              <span style={{fontSize:20}}>{icon}</span>
              <div><div style={{fontWeight:700,fontSize:13}}>{e}</div><div style={{fontSize:11,color:C.muted,marginTop:1}}>{label}</div></div>
            </button>
          ))}
        </div>
      </div>
          ...
          <table style={{width:'100%',borderCollapse:'collapse',fontSize:12,fontFamily:'Poppins,sans-serif'}}>
            <thead>
              <tr style={{background:dark?C.bg:'#F0F5FF',borderBottom:`1px solid ${C.border}`}}>
                {['Nama','Email','Role','Kategori PIC','Masa Berlaku','Status',''].map(h=>(
                  <th key={h} style={{padding:'11px 14px',fontSize:11,color:C.muted,fontWeight:700,textAlign:'left'}}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {users.map(u=>(
                <tr key={u.id} style={{borderBottom:`1px solid ${C.border}`}}>
                  <td style={{padding:'11px 14px'}}>
                    <div style={{display:'flex',alignItems:'center',gap:9}}>
                      ...
                      <div>
                        ...
                        {u.isIntern&&<span style={{fontSize:10,background:C.accentBg,color:C.accent,padding:'1px 6px',borderRadius:4,fontWeight:700}}>Intern</span>}
                      </div>
                    </div>
                  </td>
                  <td style={{padding:'6px 14px',color:C.muted,fontSize:12}}>{u.email}</td>
                  <td style={{padding:'6px 14px'}}>
                    <span style={{fontSize:11,fontWeight:700,padding:'3px 9px',borderRadius:6,background:u.role==='manager'?C.purpleBg:u.role==='pic'?C.accentBg:C.bg,color:u.role==='manager'?C.purple:u.role==='pic'?C.accent:C.muted}}>{rLabel[u.role]}</span>
                  </td>
                  ...
                  <td style={{padding:'6px 14px'}}>
                    <span style={{fontSize:11,fontWeight:700,background:u.active?C.greenBg:C.redBg,color:u.active?C.green:C.red,padding:'3px 9px',borderRadius:6}}>{u.active?'Aktif':'Nonaktif'}</span>
                  </td>
                  <td style={{padding:'6px 14px'}}>
      ...
      {tab==='add'&&(
        <div style={{maxWidth:520}}>
          <Card>
            ...
            <TInput label="Email" value={form.email} onChange={v=>setForm(f=>({...f,email:v}))} type="email" required placeholder="emailaktif@gmail.com" note="Email ini digunakan untuk login dan menerima notifikasi"/>
            <TInput label="Password Awal" value={form.password} onChange={v=>setForm(f=>({...f,password:v}))} placeholder="Minimal 4 karakter" note="Anggota disarankan mengubah password setelah login pertama"/>
